...
async def read_traj_ready(
    ...
    *,
    ...
    ``TrajReady`` is set to ``TRUE`` by :func:`~trajcenter.rws.writer.write_trajectory`
    as the final atomic signal after a full transfer.  RAPID resets it
    to ``FALSE`` once it has consumed the trajectory.
    ...
    Raises:
        RWSHTTPError: On any unexpected HTTP error from the controller.
        ValueError: If the raw value is neither ``"TRUE"`` nor ``"FALSE"``.
...
async def read_nb_robtargets(
    ...
    *,
    ...
    Returns:
        Number of robtargets currently declared on the controller (``int``).
...
async def read_traj_names(
    ...
    *,
    ...
    ABB constraints:
        ...
        - Only the first *count* entries are read — padded empty strings
          beyond *count* are never fetched.
    ...
    Raises:
        ValueError: If *count* > :data:`~trajcenter.rws.writer.MAX_TRAJ`
            or if a raw value cannot be parsed.
        RWSHTTPError: On any unexpected HTTP error from the controller.
