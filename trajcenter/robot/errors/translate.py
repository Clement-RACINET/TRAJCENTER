...
error hierarchy (:mod:`trajcenter.robot.errors.codes`). If the RW6 client
...
Design principle
-----------------
Validation failures produced locally by :mod:`trajcenter.robot.resolver`
(e.g. an out-of-bounds index, an unresolvable tool name) are raised
directly as :mod:`trajcenter.robot.errors.codes` exceptions -- they never
...
from trajcenter.robot.errors.base import TrajCenterError
from trajcenter.robot.errors.codes import (
...
def from_rws_exception(
    ...
    Returns:
        A :class:`~trajcenter.robot.errors.base.TrajCenterError` instance
        ready to be raised (with ``from exc`` at the call site) and later
        converted to a ``(code, message)`` pair via
        :meth:`~trajcenter.robot.errors.base.TrajCenterError.to_rapid`.
