...
\titre{TrajCenter v2.0}
\shortTitle{TrajCenter v2.0 -- Manuel utilisateur}
...
MODULE TRAJCENTER_PRODUCTION_EXAMPLE
    ...
    ERROR
        ...
        IF ERRNO = ERR_TJC_NOT_READY THEN

            TPWrite "Trajectory not ready";
            TPWrite "Code:" \Num:=lastErrorCode;
            TPWrite lastError;

        ELSEIF ERRNO = ERR_TJC_TIMEOUT THEN

            TPWrite "TrajCenter timeout";

        ELSEIF ERRNO =
            ERR_TJC_PROCESS_NOT_IMPLEMENTED THEN

            TPWrite "Process not implemented";
            TPWrite "Process:" \Num:=loadedProcessType;

        ELSEIF ERRNO =
            ERR_TJC_BAD_PROCESS_TYPE THEN

            TPWrite "Invalid process type";
            TPWrite "Process:" \Num:=loadedProcessType;

        ELSEIF ERRNO =
            ERR_TJC_BAD_MOVEC_PAIR THEN
...
IF found = FALSE THEN
    RAISE ERR_TJC_BAD_PROCESS_PARAM_INDEX;
ENDIF
\end{lstlisting}
