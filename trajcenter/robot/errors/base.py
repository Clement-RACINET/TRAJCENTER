...
:mod:`trajcenter.robot.errors.codes`. Translation from
``abb_rws_client_python_rw6`` exceptions into this hierarchy is handled
in :mod:`trajcenter.robot.errors.translate`.
...
class TrajCenterError(Exception):
    ...
    Every concrete error in :mod:`trajcenter.robot.errors.codes` subclasses
    ...
    Attributes:
        code: Numeric protocol status code, one of the values documented
            ...
            subclass in :mod:`trajcenter.robot.errors.codes`.
...
class TrajCenterValidationError(TrajCenterError):
    ...
    Raised by :mod:`trajcenter.robot.resolver` when a ``.trajcenter``
...
class TrajCenterAuthError(TrajCenterError):
    ...
    Raised only via :func:`trajcenter.robot.errors.translate.from_rws_exception`
